...
def concat_crash_files(path, final_file_name):
    ...
    for file_name in file_list:
        ...
        if "crashyr" in df.columns:
            df["accident_date"] = pd.to_datetime(df['crashyr'].astype(str) + '-' + df['crashmonth'].astype(str) + '-' + df['crashday'].astype(str), format='%y-%m-%d').dt.strftime('%Y-%m-%d')
        if "crash_month" in df.columns:
            df["accident_date"] = pd.to_datetime(df['crash_year'].astype(str) + '-' + df['crash_month'].astype(str), format='%Y-%B').dt.strftime('%Y-%m-%d')
        if "acc_date" in df.columns:
            df["accident_date"] = pd.to_datetime(df["acc_date"], format="%Y%m%d").dt.strftime('%Y-%m-%d')
        if "crash_date_text" in df.columns:
            df["accident_date"] = pd.to_datetime(df["crash_date_text"], infer_datetime_format=True, errors="coerce").dt.strftime('%Y-%m-%d')
        if "crash date text" in df.columns:
            df["accident_date"] = pd.to_datetime(df["crash date text"], infer_datetime_format=True, errors="coerce").dt.strftime('%Y-%m-%d')
...
for state_name in ["DE", "IA", "IL", "LA", "MD", "MN", "MT", "NY"]:
