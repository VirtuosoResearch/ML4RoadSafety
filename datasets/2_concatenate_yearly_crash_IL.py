...
def concat_crash_files(path, final_file_name):
    ...
    for file_name in file_list:
        ...
        elif "crashdate" in df.columns:
            df['accident_date'] = pd.to_datetime(df['crashdate'], format='%m/%d/%Y', errors="coerce")
            df['accident_date'] = df['accident_date'].dt.strftime('%Y-%m-%d')

        # Rename latitude and longitude columns
        if "tscrashlatitude" in df.columns and "tscrashlongitude" in df.columns:
            df = df.rename(columns={"tscrashlatitude": "lat", "tscrashlongitude": "lon"})
