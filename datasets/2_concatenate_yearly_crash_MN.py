...
def concat_crash_files(path, final_file_name):
    ...
    for file_name in file_list:
        print(file_name)
        df = pd.read_csv(path + file_name, low_memory=False, on_bad_lines='skip')
        ...
        if "crash date" in df.columns:
            df['accident_date'] = pd.to_datetime(df['crash date'], format='%m/%d/%Y %I:%M:%S %p', errors="coerce")
