...
monthly_acc_MN = np.array([7.044625,
 6.184125,
 5.006,
 4.526875,
 5.099142857142857,
 ...
 6.027285714285715,
 6.293857142857143,
 7.942428571428572])*1000

seasonal = np.array([
    monthly_acc_MN[[11, 0, 1]].sum(), # Winter
    monthly_acc_MN[[2, 3, 4]].sum(), # Spring
    monthly_acc_MN[[5, 6, 7]].sum(), # Summer
    monthly_acc_MN[[8, 9, 10]].sum(), # Fall
])
...
plt.yticks(np.arange(10000, 23000, 4000))
...
ax.set_title(r'$\mathrm{Minnesota}$', fontsize=36)
...
ax.yaxis.grid(True, lw=0.4)
plt.tight_layout()
plt.savefig('./figures/Monthly_Accidents_MN.pdf', format='pdf', dpi=100)
plt.show()